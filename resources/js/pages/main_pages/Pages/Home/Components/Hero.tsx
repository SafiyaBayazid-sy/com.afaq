import { Link } from '@inertiajs/react';
import { usePublicLanguage } from '../../../hooks/use-public-language';

export default function Hero() {
    const { dir, isEnglish } = usePublicLanguage();
    const content = isEnglish
        ? {
              imageAlt: 'Modern high-quality architectural skyscraper render',
              title: 'We build your real estate future with confidence...',
              accent: 'even when you are abroad',
              description:
                  'At Afaq Omran, we help expatriates build or upgrade their properties in Syria safely and professionally, starting with legal verification, then modern engineering studies, and finally execution using advanced construction methods and international standards.',
              contactLabel: 'Contact us',
              projectsLabel: 'Discover our projects',
              projectsIcon: 'arrow_forward',
              overlayClass:
                  'absolute inset-0 bg-gradient-to-r from-background-dark/80 via-background-dark/40 to-transparent',
              textAlign: 'text-left',
          }
        : {
              imageAlt: 'تصميم معماري حديث لبرج سكني فاخر',
              title: 'نبني مستقبلك العقاري بثقة...',
              accent: 'حتى لو كنت خارج البلد',
              description:
                  'في آفاق العمران نساعد المغتربين على بناء أو تطوير عقاراتهم في سوريا بطريقة آمنة واحترافية، بدءًا من التحقق القانوني للعقار، ثم إعداد الدراسات الهندسية الحديثة، وصولًا إلى التنفيذ بأحدث تقنيات البناء والمعايير العالمية.',
              contactLabel: 'تواصل معنا',
              projectsLabel: 'اكتشف مشاريعنا',
              projectsIcon: 'arrow_back',
              overlayClass:
                  'absolute inset-0 bg-gradient-to-l from-background-dark/80 via-background-dark/40 to-transparent',
              textAlign: 'text-right',
          };

    return (
        <section
            className="relative h-[85vh] w-full overflow-hidden"
            dir={dir}
            id="home"
        >
            <div className="absolute inset-0">
                <img
                    alt={content.imageAlt}
                    className="h-full w-full object-cover"
                    src="https://lh3.googleusercontent.com/aida-public/AB6AXuC4MLsz5h-k-vOi-RlKONbuJNKkIRVyvTgANjrgKp2-eb0RPx91-9qon9iXrcx42pDBhfxrvvoRjfVW4mr4fdE2f-RMuT5efIFtR8F4wdgmKJDwGBRGIV1Z7Rd8gRKCdQXzmBxM7M4F5KDj_TAzh4Y3xHTI2CdgsauqakdRIc1NR-CE50XUFTJe4eHEYwrcIxCTbQI-SudaQCJxphPYBY46AJ_wMbKaZVi9l3-WDntE3WQialA12UJKwOBWm2AhPGFB_cShRS8C6eVz"
                />
                <div className={content.overlayClass} />
            </div>

            <div className="relative z-10 container mx-auto flex h-full flex-col items-start justify-center px-6">
                <div className={`max-w-2xl space-y-6 ${content.textAlign}`}>
                    <h1 className="text-5xl font-black leading-tight text-white drop-shadow-lg md:text-7xl">
                        {content.title}{' '}
                        <span className="text-primary underline decoration-primary dark:text-white">
                            {content.accent}
                        </span>{' '}
                    </h1>
                    <p className="max-w-xl text-lg leading-relaxed text-slate-100 md:text-xl">
                        {content.description}
                    </p>
                    <div className="flex flex-wrap gap-4 pt-4">
                        <a
                            className="flex h-14 min-w-[180px] items-center justify-center gap-2 rounded-lg border border-white/20 bg-white/10 px-8 text-lg font-bold text-white transition-all hover:bg-white/20"
                            href="#contact"
                        >
                            <span>{content.contactLabel}</span>
                        </a>
                        <Link
                            className="flex h-14 min-w-[180px] items-center justify-center gap-2 rounded-lg bg-primary px-8 text-lg font-bold text-white transition-transform hover:scale-105"
                            href="/projects"
                        >
                            <span>{content.projectsLabel}</span>
                            <span className="material-symbols-outlined">
                                {content.projectsIcon}
                            </span>
                        </Link>
                    </div>
                </div>
            </div>
        </section>
    );
}
